...
export const Expenses: React.FC = () => {
  ...
  const [activeTabState, setActiveTabState] = useState<
    "expenses" | "dues" | "savings" | "reports" | "tasks" | "wallet" | "menu"
  >("expenses");
  const [duesActiveView, setDuesActiveView] = useState<"list" | "details">(
    "list",
  );
  const [menuSubView, setMenuSubView] = useState<"none" | "wallet">("none");
  ...
  const tabs = useMemo<
    ("expenses" | "dues" | "savings" | "reports" | "tasks" | "wallet" | "menu")[]
  >(() => ["expenses", "dues", "reports", "savings", "tasks", "menu"], []);
  ...
  const getIndicatorLeft = () => {
    ...
    const D = (containerWidth - 76) / (tabs.length - 1);
    ...
    const minLeft = 24.5;
    const maxLeft = 24.5 + (tabs.length - 1) * D;
    return Math.max(minLeft, Math.min(maxLeft, left));
  };

  const setActiveTab = (
    newTab: "expenses" | "dues" | "savings" | "reports" | "tasks" | "wallet" | "menu",
  ) => {
    const tabs: (
      | "expenses"
      | "dues"
      ...
      | "wallet"
      | "menu"
    )[] = ["expenses", "dues", "reports", "savings", "tasks", "wallet", "menu"];
    const currentIndex = tabs.indexOf(activeTabState);
    ...
    <>
      <div
        ...
        className={`flex flex-col overflow-hidden w-full ${
          isOnline
            ? "h-[calc(100dvh-92px)] lg:h-[calc(100vh-64px)]"
            : "h-[calc(100dvh-130px)] lg:h-[calc(100vh-102px)]"
        }`}
      >
        {/* 6-Icon Navigation Tabs */}
        <div className="hidden lg:block sticky top-0 z-40 bg-white/95 backdrop-blur-md h-14 text-slate-800 border-b-[1.5px] border-slate-300 shadow-[0_4px_12px_rgba(0,0,0,0.02)] select-none">
          <div
          ...
          >
            ...
            >
              <div
                className={`w-[27px] h-[27px] flex items-center justify-center transition-all relative ${
                  activeTab === "expenses"
                    ? "text-[#1a73e8]"
                    : "text-[#8e9aa8] hover:text-[#1a73e8]"
                }`}
              >
                <CustomExpensesTabIcon size={22} strokeWidth={1.5} active={activeTab === "expenses"} />
              </div>
            </button>
            ...
            >
              ...
              >
                <CustomDuesTabIcon size={22} strokeWidth={1.5} active={activeTab === "dues"} />
              </div>
            </button>
            ...
            >
              ...
              >
                <CustomBudgetIcon size={22} strokeWidth={1.5} active={activeTab === "reports"} />
              </div>
            </button>
            ...
            >
              ...
              >
                <CustomSavingsTabIcon size={22} strokeWidth={1.5} active={activeTab === "savings"} />
              </div>
            </button>
            ...
            >
              ...
              >
                <CustomTasksTabIcon size={22} strokeWidth={1.5} active={activeTab === "tasks"} />
              </div>
            </button>
        ...
        <div className="overflow-hidden w-full relative flex-1">
          ...
          >
            ...
            {/* Slide 7: Menu (Index 6) */}
            <div className={getSlideClassName("menu")}>
                          ...
                          >
                            <div className="w-10 h-10 rounded-full bg-slate-50 text-slate-600 flex items-center justify-center group-hover:bg-indigo-600 group-hover:text-white transition-all duration-300 shadow-xs mb-3.5 relative">
                              ...
                              {item.path === "/trash" && trashCount > 0 && (
                                <span className="absolute -top-1 -right-1 bg-rose-500 text-white text-[9px] font-black px-1.5 py-0.5 rounded-full min-w-[16px] h-[16px] flex items-center justify-center shadow-sm ring-2 bg-white group-hover:ring-indigo-600 transition-all animate-in zoom-in duration-300">
                                  {trashCount}
                                </span>
                              )}
                            </div>
                            <p
                              className="font-bold text-slate-800 text-xs sm:text-sm leading-tight text-center"
                              style={{ fontFamily: "'Kohinoor Bangla', sans-serif" }}
                            >
                              {item.name}
                            </p>
                            <p
                              className="font-medium text-slate-400 text-[10px] leading-tight text-center mt-1"
                              style={{ fontFamily: "'Kohinoor Bangla', sans-serif" }}
                            >
                              {item.desc}
                            </p>
      ...
      {isOnline && !isGeneratingPDF &&
        ...
        createPortal(
          <button
            ...
            className={`fixed bottom-5 lg:bottom-12 right-5 lg:right-8 text-white w-14 h-14 rounded-full flex items-center justify-center shadow-[0_4px_20px_rgba(0,0,0,0.2)] active:scale-95 transition-all duration-[150ms] ease-out z-[1000] pointer-events-auto opacity-100 scale-100 translate-y-0 ${getFabColor()}`}
            style={{
      ...
      {isModalOpen &&
        createPortal(
          <div className="fixed inset-0 z-[1000] bg-slate-900/40 backdrop-blur-xs flex flex-col items-center justify-start sm:p-4 animate-in fade-in duration-200">
            <div className="bg-white sm:rounded-[24px] w-full h-full sm:h-auto sm:max-h-[95vh] sm:max-w-lg shadow-2xl relative animate-in slide-in-from-bottom-5 duration-200 flex flex-col overflow-hidden my-auto">
                        ...
                        <input
                          ...
                          onChange={(e) => {
                            if (!isMobileScreen) {
                              ...
                              setNewExpense({ ...newExpense, amount: cleanVal });
                              if (formErrors.amount)
                                setFormErrors({ ...formErrors, amount: null });
                            }
                          }}
                          placeholder=" "
                          className={`peer w-full py-[15px] pl-[50px] pr-10 bg-transparent border rounded-2xl font-black text-rose-600 outline-none text-[17px] transition-all shadow-xs ${
                            isMobileScreen ? "cursor-pointer" : "cursor-text"
                          } ${
                            formErrors.amount
                        ...
                        <input
                          ...
                          onChange={(e) => {
                            if (!isMobileScreen) {
                              const val = e.target.value;
                              const cleanVal = val.replace(/[^0-9.]/g, "");
                              setNewIncome({ ...newIncome, amount: cleanVal });
                              if (formErrors.amount)
                                setFormErrors({ ...formErrors, amount: null });
                            }
                          }}
                          placeholder=" "
                          className={`peer w-full py-[15px] pl-[50px] pr-10 bg-transparent border rounded-2xl font-black text-emerald-600 outline-none text-[17px] transition-all shadow-xs ${
                            isMobileScreen ? "cursor-pointer" : "cursor-text"
                          } ${
                            formErrors.amount
                              ? "border-rose-500 bg-rose-50/10"
            ...
            <NumericKeypad
              isOpen={showKeypad && isMobileScreen}
      ...
      <div className="fixed inset-0 z-[200] bg-white flex flex-col w-full h-full animate-in zoom-in-95 duration-200">
        ...
        {isOnline && (
          <button
            ...
            className="fixed bottom-5 right-5 text-white bg-[#1a73e8] hover:bg-blue-600 active:scale-95 transition-all duration-150 w-14 h-14 rounded-full flex items-center justify-center shadow-[0_4px_16px_rgba(26,115,232,0.4)] z-[900]"
        ...
        {isAddTransactionModalOpen &&
          createPortal(
            <div className="fixed inset-0 bg-slate-50/95 backdrop-blur-md z-[1000] flex flex-col items-center justify-start sm:p-4">
                    ...
                    >
                      ...
                      <div className="flex gap-3">
                        {isMobileScreen ? (
                          ...
                          <div
                            className={`flex-1 relative rounded-[14px] h-[54px] transition-all duration-200 flex items-center px-4 gap-3 border bg-white ${transactionType === "receive" ? "border-[2px] border-[#3b82f6]" : "border-[#f1f5f9]"}`}
                          >
                            <span className="absolute -top-[12px] left-3 bg-white px-1.5 font-medium text-[15px] text-[#9ca3af]">
                              পেলাম
                            </span>
                            ...
                            <input
                              type="text"
                              value={transactionType === "receive" ? txAmount : ""}
                              onFocus={() => {
                                setTransactionType("receive");
                              }}
                              onChange={(e) => {
                                ...
                                const val = e.target.value;
                                const cleanVal = val.replace(/[^0-9.]/g, "");
                                setTxAmount(cleanVal);
                              }}
                              className="w-full bg-transparent outline-none font-medium text-[20px] text-slate-800"
                              placeholder="0"
                            />
                          </div>
                        )}
                        ...
                        {isMobileScreen ? (
                          ...
                          <div
                            className={`flex-1 relative rounded-[14px] h-[54px] transition-all duration-200 flex items-center px-4 gap-3 border bg-white ${transactionType === "give" ? "border-[2px] border-[#3b82f6]" : "border-[#f1f5f9]"}`}
                          >
                            <span className="absolute -top-[12px] left-3 bg-white px-1.5 font-medium text-[15px] text-[#9ca3af]">
                              দিলাম
                            </span>
                            ...
                            <input
                              type="text"
                              value={transactionType === "give" ? txAmount : ""}
                              onFocus={() => {
                                setTransactionType("give");
                              }}
                              onChange={(e) => {
              ...
              <NumericKeypad
                isOpen={isTxKeypadOpen && isMobileScreen}
